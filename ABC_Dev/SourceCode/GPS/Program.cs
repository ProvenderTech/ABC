...
namespace ABC
{
    internal static class Program
    {
        ...
        private static void Main()
        {
            ...
            {
                //opening the subkey
                RegistryKey regKey = Registry.CurrentUser.OpenSubKey(@"SOFTWARE\ABC");

                //create default keys if not existing
                if (regKey == null)
                {
                    RegistryKey Key = Registry.CurrentUser.CreateSubKey(@"SOFTWARE\ABC");
            ...
            {
                MessageBox.Show("ABC is Already Running");
            }
        }
